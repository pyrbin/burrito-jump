    ...
    where TIndex : unmanaged, IComparable<TIndex>
{
    ...
    ///     The allocator, <see cref="NativeList
    ///     <T>
    ...
    public NativeSparseList(
        NativeList<TIndex> indices,
    ...
    public NativeSparseList(TIndex[] indices, TData[] data, Allocator allocator)
        : this(indices.Length, allocator)
    {
        Indices.CopyFromNBC(indices);
        Data.CopyFromNBC(data);
    }
...
internal sealed class NativeSparseListDebugView<TIndex, TData>
    where TData : unmanaged
    where TIndex : unmanaged, IComparable<TIndex>
{
    private readonly NativeSparseList<TIndex, TData> m_List;
