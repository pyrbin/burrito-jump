    ...
    where TData : unmanaged
{
    ...
    private static readonly int DisposeSentinelStackDepth = 2;
#endif
    ...
    public NativeIntPtr NativeLength => m_Length;
    ...
    ///     The allocator, <see cref="NativeArray
    ///     <T>
    ///         "/> constructor for documentation of the
    ...
    public NativeSparseArray(int capacity, Allocator allocator)
    {
        ...
        Indices = new NativeArray<TIndex>(
            capacity,
            allocator,
            NativeArrayOptions.UninitializedMemory
        );
        Data = new NativeArray<TData>(
            capacity,
            allocator,
            NativeArrayOptions.UninitializedMemory
        );
        m_Length = new NativeIntPtr(allocator);
    }
    ...
    private int FindDataIndex(TIndex sparseIndex)
    {
        return Indices.BinarySearch(sparseIndex, 0, Length);
    }

    [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
    private void CheckFullAndThrow()
    {
        if (IsFull)
    ...
    [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
    private void CheckIndexDoesntExistOrThrow(TIndex sparseIndex)
    {
        if (ContainsIndex(sparseIndex))
...
internal sealed class NativeSparseArrayDebugView<TIndex, TData>
    where TData : unmanaged
    where TIndex : unmanaged, IComparable<TIndex>
{
    private NativeSparseArray<TIndex, TData> m_Array;
